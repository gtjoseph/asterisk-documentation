...
Much of the feature set available to content authors is actually used to create this page.  Browse the source to see how this page was created. [About-the-Documentation.md](https://github.com/asterisk/documentation/blob/main/docs/About-the-Documentation.md)
...
[Link text](sample_config.conf)
```

```markdown title="Embed a supporting image"
![Alternate text](sample_image.png)
```

#### Linking to another section in your document

Each header defined in your page is automatically assigned an "anchor" reference consisting of the header text converted to lower case with spaces replaced by dashes.  For instance the anchor for this section would be `linking-to-your-supporting-artifacts`.  If you wanted to create a link to this section in another place in your document, you'd reference it as follows:
...
Don't forget, you can link directly to a specific section in any page that has a ToC by using the heading's anchor...
...
Most of the extensions provided by [Material for MkDocs](https://squidfunk.github.io/mkdocs-material/reference/) are enabled except those only available to paying sponsors and a few that don't make sense in this environment.

Most extensions from the [PyMdown](https://facelessuser.github.io/pymdown-extensions/extensions/arithmatex/) plugin are also available.  The most useful include SuperFences (which handles code blocks) and Blocks (which handles admonitions). To see the list of extensions that are currently enabled for this site, see the "markdown_extensions" section at the bottom of the site's [mkdocs.yml](https://github.com/asterisk/documentation/blob/main/mkdocs.yml) file.
